...
pub fn sync_libs(lib_path: &PathBuf) {
    ...
    let patched_content = synclibs_file_content.replace(
        ...
        r#"$Output = Invoke-Expression -Command "${Git} fetch --quiet --all --tags --prune 2>&1"
        Write-Host "${LocalLib} trying main"
        $LatestCommit = Invoke-Expression -Command "${Git} rev-list -n1 --before=2019-12-01 main"
        If (-Not (${LatestCommit}))
        {
            Write-Host "${LocalLib} trying master"
            $LatestCommit = Invoke-Expression -Command "${Git} rev-list -n1 --before=2019-12-01 master"    
        }
        Write-Host "*** ${LocalLib} ${LatestCommit}"
